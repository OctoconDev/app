...
fun forceSquareCropState(
  ...
  val defaultRegion = src.size.toSize().let { size ->
    val target = size.minDimension
    // Calculate offsets for centering
    val offsetX = (size.width - target) / 2f
    val offsetY = (size.height - target) / 2f
    // Center
    Rect(Offset(offsetX, offsetY), Size(target, target))
  }
...
//            _region = value
  ...
  override var shape: CropShape = RectCropShape
    set(value) {}
  override var aspectLock = true
    set(value) {}
